...
export const createContact = async (req, res) => {
    try {
        const { firstName, lastName, email, countryCode, countryName, phone, subject, message } = req.body;
        ...
        // Create notification
        await createNotification('contact', `New contact message from ${firstName} ${lastName ?? ''}: ${subject || 'No subject'}`, String(newContact._id));
        res.status(201).json({
            success: true,
...
export const updateContactStatus = async (req, res) => {
    try {
        ...
        const contact = await Contact.findByIdAndUpdate(id, { status }, { new: true, runValidators: true }).select('-__v');
        if (!contact) {
            res.status(404).json({
                success: false,
                message: "Contact not found"
