...
export const createEmail = async (req, res) => {
    try {
        const { name, email, phone, message, propertyId, propertyPrice, propertyType, propertyLocation } = req.body;
        ...
        // Create notification
        try {
            await createNotification('contact', `New email inquiry from ${name}${propertyType ? ` for ${propertyType}` : ''}`, String(emailRecord._id));
        }
        catch (notifError) {
...
export const updateEmailStatus = async (req, res) => {
    try {
        ...
        const email = await Email.findByIdAndUpdate(id, { status }, { new: true, runValidators: true }).lean();
        if (!email) {
            res.status(404).json({
                success: false,
                message: 'Email not found'
