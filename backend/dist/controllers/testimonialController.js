...
export const createTestimonial = async (req, res) => {
    try {
        const { name, rating, comment, position, company, featured = false } = req.body;
        ...
        if (req.file) {
            ...
            video = await uploadToCloudinary({ buffer: req.file.buffer, mimetype: req.file.mimetype }, 'james/testimonial');
        }
...
export const getTestimonials = async (req, res) => {
    try {
        const { featured, status = 'active', page = 1, limit = 10 } = req.query;
        let filter = { status };
...
export const updateTestimonial = async (req, res) => {
    try {
        ...
        if (req.file) {
            const { uploadToCloudinary } = await import('../config/cloudinary.js');
            const video = await uploadToCloudinary({ buffer: req.file.buffer, mimetype: req.file.mimetype }, 'james/testimonial');
