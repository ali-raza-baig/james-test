...
interface CommentData {
    ...
    email: string;
    saveInfo: boolean;
    blog: string; // Changed from blogSlug to blog (ObjectId)
}
...
export const createComment = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const generateAvatar = (userName: string, userEmail: string): string => {
        ...
        await createNotification(
            'comment',
            `New comment from ${name} on a blog post (pending approval)`,
...
export const getCommentsByBlog = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!Types.ObjectId.isValid(blogId as string)) {
            res.status(400).json({
                success: false,
                message: "Invalid blog ID format"
            });
...
export const getCommentsWithBlog = async (req: Request, res: Response): Promise<void> => {
    try {
